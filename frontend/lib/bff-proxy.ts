...
async function proxy(
  ...
  authenticate: () => Promise<Awaited<ReturnType<typeof getSession>>>,
  missingAuth: string,
) {
  ...
  if (!token) {
    return bffProblem("unauthenticated", 401, missingAuth, requestId);
  }
...
export async function proxyApi(req: NextRequest, ctx: { params: Promise<{ path: string[] }> },
  authenticate: () => Promise<Awaited<ReturnType<typeof getSession>>>,
  missingAuth = 'no session cookie',
) {
