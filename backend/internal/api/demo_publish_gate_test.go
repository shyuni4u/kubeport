...
// published version can be deployed, so it is gated like publish. Deprecate is
// its other side: left open, one visitor could take a seeded template's current
// version out and nobody could deploy it or bring it back until the reset.
func TestDemoAdmin_CannotSwitchAVersionsStatusByDefault(t *testing.T) {
	s := testStore(t)
	optedIn := newDemoAdminRouterWithTemplateCreate(t, s)
	tpl := "demo-authored-" + randSuffix()
	w := do(t, optedIn, http.MethodPost, "/v1/templates", bytes.NewReader(createTemplateBody(tpl)))
	...
	requireStatus := func(want string) {
		t.Helper()
		w := do(t, gated, http.MethodGet, "/v1/templates/"+tpl+"/versions/1", nil)
		require.Equal(t, http.StatusOK, w.Code, w.Body.String())
		var v struct {
	...
	w = do(t, optedIn, http.MethodPost, "/v1/templates/"+tpl+"/versions/1/deprecate", nil)
	require.Equal(t, http.StatusOK, w.Code, w.Body.String())

	w = do(t, gated, http.MethodPost, "/v1/templates/"+tpl+"/versions/1/undeprecate", nil)
	require.Equal(t, http.StatusForbidden, w.Code, w.Body.String())
	require.Equal(t, "demo-restricted", problemShape(t, w.Body.String()).Title)
	requireStatus("deprecated")
}
