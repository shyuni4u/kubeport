...
> **▶ 현재 상태 (2026-09-17)** — 데모 가능 상태. 완료 삭제하며 갱신할 것.
> - **2026-09-17 폼 통일 (#416)**: 관리자·사용자 Input/Select/NativeSelect가 공통 스타일을 사용한다. 본문·데스크톱 입력 14px, label·보조 글자 13px, 모바일 입력 16px/44px, 기본 모서리 8px. 팀 폼은 서버 오류 후 값을 유지하며 제출 중 재전송을 막는다. 모바일 배포 메타데이터와 YAML 편집은 세로 배치. 개별 화면에서 공통 입력 스타일을 덮어쓰지 않는다.
>   - **검증**: 로컬 전체 144개 파일/1,664개 테스트와 후속 92개 테스트, 빌드·타입 검사 통과. PR/main CI·E2E·이미지 빌드·운영 배포 성공. 운영 관리자·사용자 폼과 390px 모바일 표시 확인. 운영 생성·게시·삭제 전체 수동 흐름은 미실시. 상세는 [사용 흐름 평가](docs/user-flow-review.md), 회귀 항목은 [QA 체크리스트](docs/qa-checklist.md).
> - **2026-09-16 머지분 (#407 #408 #409 #410 #411 #413)**:
>   - **디자인 방향** (#407): 민트·청회색 합의와 역할별 사용 흐름 평가를 문서화. OpenAI Apps SDK UI 기반 `pnpm design:dev`는 독립 시안이며 제품 전체 이관은 아직 아니다.
>   - **QA 결함 수정** (#408 #409 #410 → #402–#406): YAML 편집기에 템플릿·버전·상태 표시, 검색·슬라이더·로그 선택기 접근성 이름, 모바일 헤더 줄바꿈, 데모 권한 거부 시 허용 구역 안내. `nightly-job-demo`는 UTC 5분 간격으로 실패 예제를 실행하고 실행 시간·이력을 제한한다. 카탈로그 기본 스케줄은 매일 UTC 03:00 유지. 기존 시드 반영에는 리셋이 필요하다(runbook §5).
>   - **제품 가독성과 흐름** (#411): Pretendard 자체 제공, 상단 설정·폼 선택 메뉴 통일, 모바일 44px/데스크톱 40px 기본 컨트롤. 편집기는 1440px 미만 탭·넓은 화면 3패널과 하단 저장 바. 외부 주소 없는 릴리스에 다음 행동·로그 링크, 새 템플릿의 데모 제한 사전 안내, 데모 팀 생성 비활성화.
>   - **체험 진입** (#413): 랜딩의 체험 영역에 공통 비밀번호와 역할별 계정 카드를 크게 표시. 비밀번호는 문서에 복사하지 않고 화면에서 확인한다.
>   - **검증 범위와 후속**: 당시 기록상 #411·#413 CI/E2E·이미지 빌드·OCI 배포 및 운영 화면 확인 완료. 프론트엔드 141개 파일/1,645개 테스트 통과. 실제 생성→변경→삭제 전체 수동 흐름·관리자 게시는 이번 화면 점검에서 수행하지 않았다. 전체 QA 원본은 [qa-checklist](docs/qa-checklist.md)(#355), 객체형 필드 노출 후속은 #412. 평가 당시 화면과 반영 후 상태는 [사용 흐름 평가](docs/user-flow-review.md)에서 구분한다.
...
> - **2026-09-10 머지분** — 오전 #147 #141 #154 #151 #149(#134 #129 #153 #110~#112 #130, **마일스톤 A 종료**) 뒤로 오후~저녁 32커밋. 주제별로:
>   - **머지 = 배포** (#199 #218): main 머지 → 같은 sha 의 `build-images`·CI 가 둘 다 초록 → `deploy.yml` 이 `kubeport-deploy` 로 라이브 반영(첫 실측 약 8분), `/api/healthz` 에 `version`. main 이 이미 앞서 있으면 뒤처진 커밋은 건너뛰고 다음 커밋의 배포가 함께 싣는다. raw `helm upgrade` 금지·롤백 순서·Dex 가드는 아래 "멀티 세션 운영" 과 runbook §3-0.
>   - **릴리스가 남의 오브젝트를 빼앗지 않는다** (#205 → #161 #137 #125): 첫 apply 전에 소유권을 검사하고 새 kind `resource-conflict`(409), 매니페스트가 다른 ns 를 박으면 400. **시드 릴리스가 `web-app-demo` → `app-with-config-demo` 로 바뀌었다** — 방문자 대표 흐름인 `web-app` 을 비워 두려고. **시드 변경은 배포로 반영되지 않고 리셋 Job 수동 실행이 필요하다**(09-10 PM 실행 완료). 남은 한계는 #190(다중 인스턴스)·#191(검사-apply 경쟁)·#195·#196. preview 문서 상한(#160 → #135).
>   - **로그 스트림** (#168 #177 #201): 끝 프레임 `end`(#157 #162 — 3초마다 무한 재연결), 발생 시각 표시·`Last-Event-ID` 재개(#131 #107), 호출자별 동시 16개·수명 1h·5분 넘게 숨긴 탭은 스트림 반납·`http.Server` 헤더/유휴 타임아웃(#169). **데모 계정은 방문자가 공유하므로 16개도 방문자 전체가 나눠 쓴다** — 의도적 점유는 #200.
>   - **방문자 흐름** (#167 #207 #210 #213 #216 #217 #219 #222 #178): 로그아웃 확인 화면·딥링크 복귀(#28 #41), 배포 폼 구역이 데모 세션이면 `demo`·아니면 클러스터 `default_namespace`(#179), 랜딩에 데모 계정 이메일(#29), 릴리스 상세 자동 갱신(#183), 데모 제한 문구(#180), 메뉴 로그아웃 실패 표시(#166), 상대시각(#100), YAML draft 의 인스펙터 읽기 전용(#184), 마일스톤 C 대비·반응형과 ui-spec 한 글자 크래시(#113 #114 #115 #133 #164).
>   - **설치·CI·테스트** (#158 #163 #206 #214 #173 #192 #215): 클론→설치 경로(#61 #84 #89 #93 #118 #126), 리셋 Job 이 OCI 에서 Dex 에 닿게(`demo.resetHostAliasIP`), k3s 버전 고정(#194), ClusterIssuer 선행 조건(#185), **kind 테스트가 처음으로 실제 실행됨**(#121 — 빌드 태그에 가려 어디서도 돈 적이 없었다), 세션·워크트리별 테스트 DB, Playwright MCP `--isolated`.
>   - **리뷰·운영 규칙** (#171 #204 #220): `ai-reviewer` 비활성([api-agent-backlog §비활성화](docs/api-agent-backlog.md#ai-reviewer-비활성화-2026-09-10)), 멀티 세션 상시 위임·`claim:*` 선점·경계 PR 도 보안 리뷰 후 `--auto`(아래 "멀티 세션 운영").
> - **2026-09-12~13 머지분** — 머지·배포·뒷정리는 PM 세션 전담(#262). 주제별로:
...
> - **2026-09-15 머지분 (#397 #399 #400)** — 새 macOS(Apple Silicon) 머신에서 **#20 첫 클론 검증 완료**: `up.sh` 생성 경로(인증서·compose·atlas·`.env.local`·kind Ready 16초·RBAC, 81초) → `seed.sh` → `run.sh` 13 passed → `doctor.sh` → `up.sh` 재실행 2초 no-op. 거기서 나온 것:
>   - **`seed.sh` 클러스터 등록이 새 머신에서 항상 400 이었다** (#397). `common.sh` 의 `DEX_ISSUER` 는 export 안 된 셸 변수인데 접두어가 JSON 을 만드는 node 가 아니라 curl 에 붙어 `oidc_issuer_url: undefined` 로 나갔고, `curl -sf` 가 본문을 삼켜 **무출력 exit 22** 였다. 예전 머신은 셸 환경에 그 변수가 우연히 있었던 것. 이제 HTTP 코드를 검사해 Problem 본문을 `die` 로 찍고, curl 연결 실패도 조용히 끝나지 않는다.
>   - `docs/dev-setup.md` 의 `brew install … atlas-go` 는 없는 formula 라 설치 줄 전체가 중단됐다 → `atlas`. Docker Desktop 첫 기동 직후 `up.sh` 의 이미지 pull 타임아웃은 잠시 뒤 재실행(멱등) — `local-e2e.md` §0 증상표.
>   - **#355 수동 QA 는 `docs/qa-checklist.md` 가 원본**이다. 이슈 본문에 시나리오를 다시 적지 않는다 — 본문 사본이 낡아 "#354 진입점 없음", "데모 관리자가 새 템플릿 초안을 저장한다" 같은 틀린 기대가 남아 있었다(09-15 본문을 안내문으로 교체). 체크리스트는 #389(릴리스 목록 표의 상자 내 가로 스크롤)를 반영했고(#400), `local-e2e.md` §9 의 `groups` 스코프 안내는 `common.sh` 실제 동작과 맞췄다(#399).
>   - 이 머신에는 로컬 Chrome 확장이 없어 `/pr-review` 의 브라우저 페르소나 3개가 돌지 않는다(연결된 확장은 원격 Windows PC 것뿐) — 스크립트·문서 PR 이라 영향 없었고, UI 변경 PR 은 여전히 "Home PC" 인계로 본다.
...
| # | 플랜 | 상태 | 범위 |
|---|---|---|---|
| 0 | frontend-foundation | ✅ merged (PR #16) | 패키지·shadcn·Vitest·RoleBadge·StatusChip·TopBar 재편·Providers·Zustand·MonacoPanel |
| 1 | catalog-redesign | ✅ merged (PR #17) | `/catalog` + CatalogCard + 검색/태그 필터 + 아이콘 맵 |
| 2 | release-detail-redesign | ✅ merged (PR #18) | 중첩 라우트 + 개요·로그 탭 + **SSE 백엔드 추가** + k8s 용어 토글. UpdateAvailableBadge 는 Plan 3 으로 이월 (`current_version` 정수 필드·`?updateReleaseId=` 라우트 의존). |
| 3 | deploy-form-redesign | ✅ merged (PR #19) | **백엔드 3개 엔드포인트** (render/PUT releases/SSAR) + shadcn DynamicForm + RBAC 패널 + 업데이트 플로우 |
| 4 | admin-editor-redesign | ✅ merged (PR #20) | ResizablePanelGroup + MetaRow + BottomBar + SchemaTree 배지 + FieldInspector enum values + ?mode=ui\|yaml 분기 |
| 5 | backend-meta-normalization | ✅ merged (PR #21) | **MVP 전 필수 정리** — `/v1/templates/:name` JOIN 확장, `values_json` RawMessage, `PATCH /v1/templates/:name` 신설, 배포 폼 클러스터 드롭다운. Plan 3·4 구현 중 발견된 백엔드 구멍들. |
| 6 | post-mvp-stabilization | ✅ merged (PR #27) | 첫 실제 브라우저 테스트에서 발견된 버그·UX 갭 정리 — 레거시 `/templates/[name]/edit` 제거, 상세 페이지 permission-aware UI + `+ 새 버전`·`삭제`, `/templates/new` + version-edit 모드 탭, yaml→UI 변환 + 사용자 폼 preview, 릴리스 리스트 `상태` 컬럼 제거. **백엔드 추가**: `PATCH`/`DELETE /v1/templates/:name/versions/:v` (drafts-only), core-API openapi 경로 수정, `ui_state_json` RawMessage, `TestMain` cleanup. |
| 7 | plan7-visual-refresh | ✅ merged (PR #28) | **비주얼 리프레시 + i18n(ko/en) — 기능·라우트·백엔드 변경 없음.** Figma 레퍼런스(`nDP3cHNKf5Cjo6F2HU1EVv`) 기반 디자인 토큰 재정의 + `AppShell`(좌측 사이드바 + 얇은 탑바) 도입 + `CatalogCard`/`ReleaseTable`/`RoleBadge`/`StatusChip`/`MetricCards` 비주얼 리프레시 + `next-intl` 로 사용자향 문자열 외부화 + 로케일 토글 UI. |
| 8 | plan8-release-stale-cleanup | ✅ merged (PR #35) | **DB ↔ k8s drift 회수 (Stage 1).** `GetRelease` 가 `cluster-unreachable` / `resources-missing` 분리 (read-time, DB write 없음). `DELETE /v1/releases/:id?force=true` (admin 전용) 로 k8s 호출 건너뛰고 DB row 만 정리. 릴리스 상세에 explainer 배너 — 일반 사용자에겐 "관리자에게 문의", admin 에겐 강제 삭제 버튼. ko/en i18n. **Stage 2 (reconcile loop, SA 토큰, DB 컬럼) 는 Plan 12 (deferred) 로 분리.** |
| 9 | plan9-helm-chart | ✅ 라이브 사용 중 | **Helm chart MVP — Plan 10 의 하드 블로커.** `deploy/helm/kubeport/` 단일 chart 에 backend / frontend / 옵셔널 in-cluster Postgres / Ingress / cert-manager Certificate / atlas migration Job 구성. 클라우드 중립 (Ingress class · StorageClass · 도메인만 values 분기). CI: `helm lint` + golden snapshot + kind smoke. 코드 변경 없음 (chart 만 추가). 추정 공수 2.5–3 영업일. |
| 10 | plan10-oci-phase2-bootstrap | ✅ 실행 완료 (2026-08-20) | **OCI Phase 2 직행 부트스트랩 — 라이브 `kubeport.enzo.kr`.** OCI A1(춘천) + 도메인 + cert-manager/Let's Encrypt + Google OAuth + helm install. **추가로 실제 배포 활성화**: k3s Google OIDC 신뢰 + iptables pod/service CIDR + RBAC + 클러스터 `oci-a1` 등록. 운영·롤백·함정: [oci-prod-runbook](docs/oci-prod-runbook.md) + [deploy/oci/README §7](deploy/oci/README.md). GCP Phase 1 은 OCI capacity 조기 확보로 **건너뜀**. |
| 11 | [docs/local-e2e.md §9c](docs/local-e2e.md) | 🚧 1차 (2026-09-08) | **e2e 확장.** 로컬/CI kind 대상 스펙 3개 추가 — `05-user-deploy`(폼 검증·RBAC 거부 문장·배포·삭제), `06-admin-draft-save`(미저장 가드·PATCH 저장), `07-error-pages`(not-found). **남은 것**: 라이브 OCI smoke(`demo` ns 검증 릴리스 생성·삭제는 2026-09-10 부터 상시 위임 — "멀티 세션 운영"), 클러스터 끊김·drift 회수 케이스(kind 에서 재현 방법 필요). |
...
| 13 | plan13-demo-mode | ✅ 라이브 (PR #2, 2026-09-08 롤아웃) | **데모 모드.** Dex 데모 IdP + 데모 계정 2개(관리자/사용자) + 시드(템플릿 3·릴리스 2) + 리셋 CronJob(현재 하루 1회) + k3s 구조화 인증(Google+Dex) 전환 스크립트. 스펙: [self-improving-loop-design](docs/superpowers/specs/2026-09-07-self-improving-loop-design.md) §4.1. |
...
- **dependabot PR** — PM 이 CI 전체(playwright 포함) 초록을 본 뒤 머지한다. `github-actions`·`docker` 갱신은 릴리스 노트에서 권한·네트워크 변경을 한 번 훑고 머지한다

**fork PR(외부인)** 은 머지하지 않고 사용자에게 한 줄로 알린다 — 코드 출처가 이 세션들이 아니고, 머지가 곧 배포다.
