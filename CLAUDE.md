...
> - **Plan 11 완료 (2026-09-17, PR #423)**: 로컬·CI 실제 kind에서 장애·복구와 로그 인스트림 `rbac-denied`까지 검증. 운영 OCI 생성·로그·삭제 smoke 및 잔여 객체 없음 확인. [검증 기록](docs/plan11-verification.md). **남은 로드맵**: Plan 12(reconciler, 보류), Plan 14(UX 루프), Plan 15(기록 자동화). 리뷰어 백로그의 현재 목록은 `gh issue list --label reviewer`로 확인한다.
...
| 11 | [docs/local-e2e.md §9c](docs/local-e2e.md) | ✅ 검증 완료 (2026-09-17, PR #423) | **E2E 확장.** 로컬·CI kind 전체 15개 통과. 전용 TCP 프록시로 연결 장애·복구, 로그만 RBAC 거부·복구, 리소스 유실 감지, 일반 사용자 강제 삭제 거부와 관리자 정리 검증. 운영 OCI demo에서 배포·정상 상태·실제 로그·삭제·잔여 리소스 없음 확인. [검증 기록](docs/plan11-verification.md). |
