...
> **▶ 현재 상태 (2026-08-20)** — 데모 가능 상태. 완료 삭제하며 갱신할 것.
...
> - **남은 것**: (a) **`kubeport`→`kubeport` 네이밍 통일**(이미지·차트·리소스명), (b) `deploy/oci/bootstrap.sh` 에 §7 인프라 수정(iptables pod/service CIDR · OIDC config) 반영해 재현성 확보, (c) 데모용 첫 템플릿 시드, (d) Plan 11 e2e 확장(이제 라이브 OCI 대상).
