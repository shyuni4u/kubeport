...
### 데모 체험과 개인 계정 로그인은 어떻게 다른가요?

| 구분 | 데모 체험 | 개인 계정 로그인 |
| --- | --- | --- |
| 계정 | 첫 화면에 공개된 공용 데모 계정 | 설치에 연결된 인증 제공자(OIDC)의 본인 계정. 공개 사이트에서는 Google |
| 목적 | 사용자·관리자 화면과 제한된 배포 흐름 체험 | 해당 설치에서 본인에게 허용된 작업 수행 |
| 권한 | 데모 구역과 기능 제한 적용. ‘관리자로 체험’도 전체 관리자 권한을 제공하지 않음 | kubeport의 관리자·팀 권한과 대상 Kubernetes RBAC에 따라 결정 |
| 데이터 | 다른 방문자와 공유하며 매일 초기화. 실제 비밀번호·API 키·개인정보 입력 금지 | 개인 계정 DB 기록은 데모 정리 대상에서 제외되지만, 데모 구역의 리소스는 계정과 무관하게 초기화. 그 외 보관·삭제는 설치 운영 정책에 따름 |

**Google로 로그인해도 개인 전용 클러스터나 관리자 권한이 자동으로 생기지 않습니다.** 데모와 개인 계정 로그인은 같은 설치에서 제공하는 서로 다른 로그인 경로이며, 로그인 버튼이 별도의 서버나 DB로 이동한다는 뜻은 아닙니다. 회사·개인 인프라에서 실제로 사용하려면 [직접 설치](#클러스터에-설치하기) 안내에 따라 kubeport를 설치하고 인증·팀·클러스터 권한을 설정하세요.

**데모 기능을 끈 설치(`demo.enabled=false`)에는 데모 리셋 작업이 생성되지 않습니다.** 데모가 켜진 설치에서는 DB의 데모 계정 소유 데이터를 정리하고, 데모 네임스페이스의 워크로드·Secret·PVC 등은 생성한 계정과 무관하게 삭제합니다. 개인 계정으로 로그인했더라도 데모 구역에 배포한 리소스는 초기화되므로, 실제 업무용 배포와 데이터는 별도 구역에서 운영하세요.

데모의 주기적 초기화는 체험 데이터를 정리하는 절차입니다. 사용자 구분이나 접근 권한 검사를 대신하지 않습니다.
