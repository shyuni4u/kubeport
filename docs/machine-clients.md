...
- 얻은 신원은 데모 도메인이라 다음에서 **403 `demo-restricted`** 다: 항상 막히는 관리 쓰기(`POST /v1/clusters`,
  ...
  `POST /v1/templates/:name/versions/:v/publish`·`/undeprecate`(템플릿과 무관하게 같은 403 — [#294](https://github.com/shyuni4u/kubeport/issues/294), 초안 작성은 열려 있다), 그리고 데모 계정이 만들지 않은
  릴리스의 읽기·변경.
