...
- 얻은 신원은 데모 도메인이라 다음에서 **403 `demo-restricted`** 다: 항상 막히는 관리 쓰기(`POST /v1/clusters`,
  ...
  `POST /v1/templates/:name/versions/:v/publish`·`/deprecate`·`/undeprecate`(템플릿과 무관하게 같은 403 — [#294](https://github.com/shyuni4u/kubeport/issues/294), 초안 작성은 열려 있다), 그리고 데모 계정이 만들지 않은
  릴리스의 읽기·변경.
...
`log` 프레임에는 SSE `id:` 가 붙는다(kubelet 스탬프가 없는 줄 제외). **값의 모양은 스트림이 무엇을
따라가느냐에 따라 다르다.**

**`?instance=<파드>` 로 연 스트림** — 그 줄이 쓰인 시각:
...
**`instance=all`(기본값)** — 파드별 커서. 위치가 있는 파드마다 `파드@시각` 을 파드 이름순으로 쉼표로
이어 붙인다([#172](https://github.com/shyuni4u/kubeport/issues/172)):

```
id:web-1@2026-09-09T07:36:36.123456789Z,web-2@2026-09-09T07:35:00.900000000Z
event:log
data:{"time":1788939396123,"pod":"web-1","text":"..."}
```

커서는 방금 줄을 쓴 파드만이 아니라 **이 스트림에서 위치가 생긴 모든 파드**(받은 커서에 있었거나 스탬프 있는
줄을 한 번이라도 쓴 파드)의 위치를 담는다. 브라우저는 마지막 `id` 하나만 기억하기 때문이다. 아직 아무 줄도
안 쓴 파드는 빠지고, 재연결하면 그 파드는 처음부터 받는다.

**시각 형식은 고정이다 — 항상 UTC, 소수 정확히 9자리.** 그래서 한 파드의 시각끼리는 문자열 순서와 시간
순서가 같다. 단일 인스턴스라면 받은 값 중 가장 큰 것을 커서로 골라도 안전하다. **`all` 커서는 문자열로
비교하지 말고 마지막으로 받은 것을 통째로 돌려준다.** 직접 만들 때도 같은 형식을 쓴다(`?since=` 의 시각은
자릿수·오프셋이 달라도 받아 준다).

**`time` 은 그 줄을 컨테이너가 쓴 시각(unix ms)이다** — 단일 인스턴스 스트림에서는 `id` 를 밀리초로 자른
값이고, `instance=all` 에서는 커서 안의 그 파드(`pod`) 시각을 밀리초로 자른 값이다. #131 이전에는
...
`EventSource` 를 안 쓰는 호출자는 마지막으로 받은 `id` 를 그대로 `?since=` 로 넘긴다(쿼리 문자열로
인코딩해서). 둘 다 있으면 `?since=` 가 이긴다.

**이름을 지정하면 그 시각에서 재개된다.** 이름은 요청에 들어 있으므로 재연결해도 같은 파드를 가리키고,
그 파드가 사라졌다면 다른 파드가 그 위치를 물려받는 대신 `404 no-pods` 가 온다.

**`instance=all` 은 파드마다 자기 위치에서 재개된다.** 시각 하나로는 안 된다 — 파드들은 동시에 따라가며
순서 없이 합쳐지므로 가장 최근 줄은 **마지막으로 쓴 파드의 것**이고, 그 시각으로 전 파드를 재개하면 느린
파드가 아직 못 흘린 구간이 **조용히, 영구히** 건너뛰어진다. 그래서 커서가 파드별 위치를 담는다.

`all` 이 가리키는 파드 집합은 연결 사이에 바뀔 수 있다:

- 커서에 **없는 파드**(그사이 새로 뜬 파드)는 처음부터 받는다.
- 커서에 있는데 **사라진 파드**의 위치는 버린다. 레플리카 1개짜리가 롤아웃되면 떠난 파드의 위치가
  그 자리를 대신한 파드에 적용되면 안 되기 때문이다.
- 파드가 **8개를 넘으면** 커서가 프레임마다 실리므로 커서를 내주지 않고, 받은 커서도 무시한다 — 예전처럼
  처음부터 다시 받는다. 이때 스트림은 **맨 앞에 `replay` 프레임**을 보내니 받아 둔 줄을 버린다(`id` 없는
  프레임만 보고는 알 수 없다 — `EventSource` 는 id 없는 이벤트에도 직전 id 를 그대로 싣는다). 스탬프 있는
  줄의 `id` 는 `-` 다. 커서가 아니라서, 파드 수가 다시 8 이하로 줄어든 뒤 재연결해도 옛 커서로 재개하지 않고
  `replay` 와 함께 처음부터 받는다.
- **`replay` 프레임 자신도 `id:-` 를 싣는다.** 받아 둔 줄을 버리는 순간 들고 있던 재개 지점도 함께 버려야 하기
  때문이다 — 그 직후 첫 줄이 오기 전에 끊기면 옛 커서가 다시 적용돼 비운 패널에 없는 줄을 건너뛴다.
  `-` 는 `Last-Event-ID` 로도 `?since=-` 로도 그대로 돌려주면 된다(두 보기 모두 400 이 아니라 `replay` 와 함께
  처음부터).

`instance=all` 에 **시각 하나짜리** `?since=` 를 주면 **`400 validation-error`** 다 — 그 값을 전 파드에
적용하는 것이 바로 줄을 잃는 재개다. (`Last-Event-ID` 헤더에 온 시각 하나는 거절하지 않고 무시한다.
브라우저가 인스턴스를 바꿔도 같은 `EventSource` 에 남겨 두는 값이라서다.)
...
- `?since=` 가 깨졌으면 `400 validation-error` 다. 호출자가 쓴 값이고, 전체 로그로 답하면 무시당한
  줄 모르고 계속 보내게 된다.
- `Last-Event-ID` 가 깨졌으면 **에러가 아니라 `replay` 프레임 뒤에 처음부터 보낸다.** 그 값은 아무도 타이핑하지 않았다 —
...
- 필요 없어진 스트림은 닫는다. 상한은 호출자별이라 **데모의 공유 계정은 이 상한도 공유**한다. 그래서 데모 계정에는
  **로그인 한 번마다 8개**라는 상한이 따로 있다([#200](https://github.com/shyuni4u/kubeport/issues/200)) — 방문자 한 명이 공유 상한을
  다 차지하지 못하게. 넘으면 같은 `429 too-many-streams` 다. 다시 로그인하면 새 상한을 받으므로, 막는 장치가 아니라 차지하는 비용을
  올리는 장치다.
...
**재연결**하면 된다 — `EventSource` 는 알아서 한다. 이름 지정 인스턴스는 그 시각에서, `instance=all`
은 파드별 커서에서 재개된다(파드가 8개를 넘으면 처음부터).
...
### 노드 화면의 eviction 권한은 파드마다 다르다 ([#437](https://github.com/shyuni4u/kubeport/issues/437))

`GET /v1/clusters/{name}/operations?area=nodes` 는 **모든 네임스페이스**의 파드를 나열하므로
`namespace` 쿼리를 **무시한다**. 각 파드의 `evictable` 이 그 파드 자기 네임스페이스에서의
`pods/eviction` 권한이고, **필드가 없으면 거부가 아니라 미상**이다 — 한 번에 최대 64개
네임스페이스만(목록 순서 = 네임스페이스 오름차순) 검사하므로 그 뒤는 검사되지 않는다.
미상인 파드로 `evict` 를 보내면 거부는 클러스터가 하고 `403 rbac-denied` 로 온다.

최상위 `permissions.evict` 는 **나열된 네임스페이스가 전부 거부했을 때만** false 인 화면 단위
힌트라, 개별 행동의 게이트로 쓰면 안 된다. **#437 이전에는 쿼리 `namespace` 하나의 답이었다** —
그때 값을 행동 게이트로 쓰던 클라이언트는 한 네임스페이스라도 허용되면 true 를 받으므로,
파드별 `evictable` 로 옮겨야 한다.
