...
`log` 프레임마다 SSE `id:` 가 붙는다. 값은 그 줄이 쓰인 시각(나노초)이다:

```
id:2026-09-09T07:36:36.123456789Z
event:log
data:{"time":1789284996123,"pod":"web-...","text":"..."}
```
...
**단, 파드 하나를 따라갈 때만이다.** `instance=all` 은 `id` 를 안 붙이고 재개 지점도 무시한다 —
예전처럼 처음부터 다시 받는다. 시각 하나는 파드 여러 개의 진행을 대표할 수 없기 때문이다: 파드들은
동시에 따라가며 순서 없이 합쳐지므로 가장 최근 `id` 는 **마지막으로 쓴 파드의 것**이고, 그걸로 전
파드를 재개하면 느린 파드가 아직 못 흘린 구간이 **조용히, 영구히** 건너뛰어진다. 줄을 다시 보내는
쪽이 싸다. 파드별 커서는 [#172](https://github.com/shyuni4u/kubeport/issues/172) 으로 따로 본다.
...
파싱할 수 없는 재개 지점은 `400 validation-error` 다. 전체 로그로 답하면 무시당한 줄 모르고 계속
보내게 된다.
