...
## 이후 반영된 폼 개선 (2026-09-17, #416)

[PR #416](https://github.com/shyuni4u/kubeport/pull/416)은 관리자·사용자 폼의 label/값 위계, 입력·선택 컨트롤과 버튼 모서리, 글자 크기를 통일했다. 관리자 메타데이터·속성·팀 폼에 공통 컴포넌트를 적용하고, 좁은 화면의 YAML 에디터와 사용자 배포 메타데이터를 세로로 배치했다. 팀 폼은 서버 오류 후 입력값을 유지하고 제출 중 중복 요청을 막는다. 아래의 9월 16일 표·캡처는 당시 관찰 기록으로 유지한다.

검증 근거는 다음과 같이 구분한다.

- 로컬: 전체 144개 파일/1,664개 테스트 및 후속 관련 92개 테스트, 프로덕션 빌드·타입 검사 통과. ESLint 오류 0, 기존 경고 3. fixture 화면에서 필수값 오류·복구, 키보드 조작, 관리자 필드 편집과 사용자 미리보기 일치, 밝은/어두운 테마를 확인했다.
- 자동화: PR과 main CI·Playwright E2E, 운영 이미지 빌드·[자동 배포](https://github.com/shyuni4u/kubeport/actions/runs/35171338733) 성공.
- 운영 화면: 배포 직후 health `status=ok`, 당시 버전 `0c0cd5f` 확인. 새 템플릿과 실제 사용자 배포 폼에서 label 13px/입력 14px/모서리 8px, 390px 모바일에서 입력 16px/높이 44px·한 열 배치·가로 넘침 없음을 실측했다. 클러스터·미리보기·권한 확인 정상 로딩과 검증용 데모 세션 로그아웃도 확인했다. 이 버전은 당시 검증 대상이며 현재 운영 버전은 health 응답으로 확인한다.

운영 데이터 생성·변경·게시·삭제는 실행하지 않았다. 따라서 앞서 적은 전체 수동 흐름·팀 권한 변경·복수 클러스터의 미검증 범위는 유지한다. 공통 폼의 재발 방지 기준은 [디자인 기준](design-direction.md), 실행 항목은 [QA 체크리스트](qa-checklist.md)를 따른다.
