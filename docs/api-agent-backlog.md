...
> **2026-09-17 방향 변경:** 사용자 요청으로 [Skill + 로컬 CLI](ai-client.md)를 제공한다.
> 각 설치본의 기존 브라우저 로그인으로 연결 토큰을 발급하며 중앙 인증/MCP 서비스는 운영하지 않는다.
> 아래의 “실제 호출자가 없다”는 설명과 리뷰어 중단 사유는 당시 기록이다. 이제 CLI가 실제 호출자이므로
> 필요한 계약 개선은 개별적으로 승격한다. 기존 1~7 항목 전부를 AI 지원의 선행 조건으로 삼지 않는다.
...
> **2026-09-14 기준** 위 "열림" 4건은 모두 고쳐져 닫혔다: #74(PR #290 목록 필터 구현, 모르는 쿼리 400),
> #107(PR #177 로그 재개), #127(PR #293 트레일링 슬래시 예외 문서화), #128(PR #266 `413 payload-too-large`).
...
- [x] 열린 채 남은 4건(#74 #107 #127 #128)이 그때까지 살아 있는지 먼저 확인한다 — 2026-09-14 확인, 전부 닫혔다(위 표 아래).
...
**부분 구현됨 — PR #347([#340](https://github.com/shyuni4u/kubeport/issues/340)).** `GET /v1/releases/{id}?include=storage_on_delete`
가 삭제와 같은 셀렉터로 클러스터를 읽어, 이 릴리스를 지우면 StatefulSet 저장소(PVC)가 함께 지워지는지를 `deleted`·`kept`·`none`·
`unknown` 으로 답한다. 웹 UI 의 삭제 확인 문구가 이 값을 쓴다. 남은 것: 지워질 오브젝트 목록, `?dry_run=`·확인 토큰, 삭제 응답이
지운 것을 돌려주기. 판정과 `DELETE` 사이에 업데이트가 끼면 답이 낡을 수 있다.
...
type 오타가 검증 없이 통과하던 문제. 실제 버그라 이 문서가 아니라 이슈로 다뤘고 **구현됨(PR #223)**: 저장할 때 모르는 `type`
을 거절하고, 그 검사 이전에 저장된 버전은 값이 있을 때 배포·업데이트·`/render` 가 `400` 이다.
...
집계해도 그 어휘가 안정적이라는 보장이 없다. 카탈로그의 태그 칩은 프론트가 로드된 템플릿에서 파생한다.

**부분 구현됨 — PR #290([#74](https://github.com/shyuni4u/kubeport/issues/74)).** `GET /v1/templates?tag=<t>` 서버 필터가
생겼다(반복하면 모든 태그를 가진 것만 남긴다). 남은 최소안: openapi 에 태그의 `maxItems`/`maxLength`/`pattern` 선언.
