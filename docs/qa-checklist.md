...
- [ ] 템플릿의 입력 폼이 그리는 도중 실패하면 흰 화면이 아니라 빨간 안내 "이 템플릿의 입력 폼을 그리지 못했습니다. 템플릿에 문제가 있을 수 있으니 관리자에게 알려 주세요." 가 폼 자리에 한 번만 보이고, 내부 원인 문구는 보이지 않으며, 배포 이름·클러스터·구역 입력과 미리보기는 그대로 남는다. 배포 이름을 입력해도 안내가 사라졌다 다시 뜨지 않고, 다른(정상) 템플릿 spec 을 받으면 폼이 다시 그려진다 (#188 — 실제로 폼을 깨뜨리는 템플릿이 알려져 있지 않아 눈으로는 못 보고 `frontend/app/catalog/[name]/deploy/DeployClient.crash.test.tsx` 가 자동 검증)
