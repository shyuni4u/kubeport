# kubeport 디자인 기준 — Mint workspace

2026-09-16 사용자와 합의한 시각적 방향. 이후 UI 작업은 이 문서를 기준으로 시작한다.

## 합의한 느낌

사용자가 제공한 Codex 데스크톱 화면의 옅은 민트 사이드바, 청회색 선택 배경, 얇고 작은 아이콘, 흰색으로 떠 있는 둥근 서브메뉴를 선호했다. 이전의 베이지·웜그레이 제안은 선호하지 않았다. 아래 이미지는 이 피드백을 반영한 마지막 시안이며, 세부 레이아웃 전체의 제품 적용까지 확정한 것은 아니다.

![선호한 민트 방향](design-assets/mint-direction.png)

원본 개인 작업 화면은 저장소에 복사하지 않는다. 이미지의 문구·아이콘·비율은 콘셉트 참고이며 실제 접근성·반응형 구현의 사양을 대신하지 않는다.

## 공식 컴포넌트 우선

기본 컨트롤을 새로 복제하지 않고 **OpenAI `@openai/apps-sdk-ui` 0.2.2**를 재사용한다. 공식 [설명](https://developers.openai.com/plugins/build/chatgpt-ui#optional-openai-component-library), [소스](https://github.com/openai/apps-sdk-ui), [컴포넌트 미리보기](https://openai.github.io/apps-sdk-ui/), [MIT 라이선스](https://github.com/openai/apps-sdk-ui/blob/main/LICENSE).

이 패키지는 ChatGPT 앱용 공개 React 디자인 시스템이다. Codex 데스크톱의 내부 구현과 동일하다는 근거는 확인하지 않았다. React 18/19, Tailwind 4를 요구하며 OpenAI API 연결이나 API 키 없이 시안의 UI를 렌더링할 수 있다.

| 재사용 | kubeport에서 조합 |
| --- | --- |
| Button, Input, Badge, Checkbox, Slider, Popover, Icon | 민트 사이드바, 카탈로그 목록·상세 배치, 상태 문구, 업무 흐름 |
| 라이브러리 기본 토큰과 상호작용 | `--kp-*`로 분리한 앱 표면 색상과 간격 |

현재 제품은 shadcn/Base UI를 사용한다. 시안 PR #407은 공개 패키지를 **개발 의존성**으로 추가하고 독립 Vite 시안에서만 사용했다. #407은 프로덕션 전역 CSS·인증·배포 기능을 변경하지 않았으며, 이후 #411에서 아래의 제품 가독성 개선을 적용했다. Apps SDK UI를 실제 제품에 적용할 때는 전역 CSS 충돌, 포털 테마, 폼 어댑터, 키보드 접근성, 번들 크기를 비교한 뒤 화면 단위로 옮긴다. 두 디자인 시스템을 무작정 전역에 동시에 로드하지 않는다.

0.2.2의 Slider는 숫자 입력에만 라벨을 연결하므로 별도 손잡이에도 접근성 이름을 연결하는 작은 어댑터를 사용한다. “공식 패키지”라는 사실이 제품에 통합한 결과의 접근성 검증을 대신하지 않는다. 라이선스 고지는 패키지에 포함되어 있으며, 정적 결과를 배포할 때도 OpenAI MIT 고지를 보존한다.

## 시안 실행

저장소 루트에서:

```powershell
cd frontend
pnpm install --frozen-lockfile
pnpm design:dev
```

[로컬 시안](http://127.0.0.1:4173/)에서 카탈로그 선택, 검색과 빈 결과, 배포 연습 대화상자, 환경 팝오버, 밝기 전환, 버튼 피드백, 입력, 수량, 체크박스와 상태 배지를 확인한다. 로그인과 백엔드가 필요 없다. 실제 리소스를 생성하거나 설정을 저장하지 않는다. 새로고침하면 초기화된다. 언어는 이 합의용 시안에 한해 한국어다.

`pnpm design:build`는 `frontend/out/design-preview`에 정적 빌드한다. 개발 서버는 루프백 주소만 사용하며 서비스 자동 배포에는 포함되지 않는다. 소스 진입점은 [main.tsx](../frontend/design-preview/main.tsx), 스타일은 [style.css](../frontend/design-preview/style.css)다.

![공식 UI 패키지로 구현한 데스크톱 시안](design-assets/preview-desktop.png)

[모바일 캡처](design-assets/preview-mobile.png) · [입력 검증 캡처](design-assets/preview-user.png) · [환경 선택 확인](design-assets/preview-admin.png)

검증: 타입 검사·시안 빌드·디자인 코드 ESLint 통과, 기존 프론트엔드 137개 파일/1,624개 테스트 통과. 브라우저에서 검색·빈 결과·템플릿 선택·잘못된 이름 차단·정상 연습 완료·환경 선택·키보드 슬라이더/체크박스·Escape 복귀를 확인했다. 1440×900 및 390×844에서 라이트/다크와 열린 메뉴를 확인했고 페이지 가로 넘침은 관찰되지 않았다. 실제 프로덕션 생성·게시 검증은 포함하지 않는다.

## 앱 표면 토큰

| 역할 | 라이트 |
| --- | --- |
| 본문 표면 | `#FFFFFF` |
| 사이드바 | `#EDF5F3` |
| 내비게이션 선택 | `#DFEAE8` |
| 콘텐츠 선택 | `#EAF3FD` |
| 기본 텍스트 | `#373E40` |
| 보조 텍스트 | `#626E72` |
| 포커스·링크 | `#2563EB` |

큰 그림자와 보라색 강조를 반복하지 않는다. 공간·정렬·글자 크기로 먼저 구분하고, 구분선은 보조로 사용한다. 아이콘은 공식 Icon 패키지에서 가져온다. 메뉴와 대화상자에만 높이감을 준다. 연한 배경은 장식이며, 선택 여부는 `aria-pressed`, 테두리 등으로도 표시한다. 다크 모드는 사용자 원본에 없는 확장 제안이다.

## 적용 전에 지킬 것

### 제품 가독성 개선 (2026-09-16)

B2B 업무 도구라는 사용자 기준에 맞춰 작업 밀도와 예측 가능한 동작을 우선한다. 소비자 서비스 수준의 장식·온보딩 확장은 우선하지 않는다. 선택 메뉴는 상단 설정과 배포 폼에서 기존 Base UI Select를 공유하고, 8px 트리거 모서리·40px 옵션 행·선택 체크·키보드 포커스를 일관되게 제공한다.

제품 본문은 사용자 선호에 따라 Pretendard Variable 1.3.9를 자체 제공한다. 외부 폰트 CDN 요청 없이 `next/font/local`로 로드하며, SIL OFL 라이선스를 폰트와 함께 보관한다. 코드와 로그는 기존 모노스페이스를 유지한다. label·보조 글자는 13px, 데스크톱 본문·컨트롤 글자는 14px(본문 행간 20px), 모바일 입력값은 16px, 기본 컨트롤 높이는 모바일 44px·데스크톱 40px를 기준으로 한다.

관리자 편집기는 1440px 미만에서 탭으로 전환하고, 넓은 화면은 각 패널에 최소 30%를 배정한다. 패널 높이를 제한하고 저장 바를 화면 하단에 붙여 긴 스키마에서도 저장 동작을 찾을 수 있게 한다. 이 가독성 개선은 기존 제품 컴포넌트에 적용하며, 위의 민트 시안과 Apps SDK UI 전체 이관은 별도다.

- 배포 구역·클러스터는 제출 전에 명확히 보여준다. 모든 중요 정보를 환경 메뉴 안에만 숨기지 않는다.
- 상태 색상을 줄이더라도 정상·준비·실패·미확인을 문구와 아이콘으로 구분한다.
- 비전문 사용자에게 Deployment 등의 리소스 목록을 먼저 읽도록 강요하지 않는다. 실제 제품에서는 업무 설명을 먼저, 기술 상세를 보조로 검토한다.
- 제품 전체 재설계는 독립 시안 #407과 별도다. 기존 이슈 #402–#406의 수정은 같은 날 #408–#410에 반영됐다.

### 체험 진입과 다음 행동 (2026-09-16, #411·#413)

랜딩은 체험 진입을 앞에 두고, 공통 비밀번호와 역할별 계정·버튼을 하나의 체험 영역에 묶는다. 이메일·비밀번호는 전체 선택으로 복사할 수 있게 하며, 실제 비밀번호는 문서나 캡처 예시에 고정하지 않는다. 외부 주소 없는 릴리스에는 다음 행동 안내와 해당 릴리스의 로그 링크를 제공한다. 데모 관리자는 새 템플릿 편집 전에 저장·게시 제한을 읽을 수 있고, 팀 생성 입력과 버튼은 비활성화된다. 외부 접속 URL 자동 제공은 이 변경에 포함되지 않는다.

사용 흐름의 평가와 다음 개선 순서는 [사용 흐름 평가](user-flow-review.md)를 참고한다.

### 관리자·사용자 폼 공통 규칙 (2026-09-17, #416)

관리자 화면과 사용자 배포 폼은 같은 입력 스타일을 사용한다. `components/ui/field-styles.ts`가 높이(데스크톱 40px, 모바일 44px), 모서리(8px), 배경, 값의 글자색, 포커스·오류·비활성 상태를 정의하며 Input, SelectTrigger, NativeSelect가 공유한다. Label/FormLabel과 관리자 메타데이터·속성 label도 같은 글자 크기와 색상 규칙을 따른다. 이름표는 입력 위에 계속 보이게 두며 placeholder로 대체하지 않는다.

관리자는 메타데이터·리소스 구조·속성·사용자 미리보기의 작업 구역을 유지한다. 새 템플릿의 소유 팀은 메타데이터와 같은 구역에 둔다. YAML 두 파일은 좁은 화면에서 세로로 배치한다. 팀 생성·멤버 추가는 같은 폼 규칙을 따르고, 서버 오류가 나면 입력값을 유지한다. 제출 중 재전송 및 데모에서 불가능한 멤버 변경을 막는다.

[PR #416](https://github.com/shyuni4u/kubeport/pull/416)으로 운영에 반영했다. 속성의 입력 모드·선택지는 fieldset/legend로 묶으며, 개별 화면에서 공통 입력의 글자 크기·모서리·여백을 덮어쓰지 않는다. 사용자 배포 메타데이터는 모바일 한 열, 넓은 화면 두 열로 배치한다.

검증은 로컬 fixture 화면과 배포 후 운영 화면으로 구분한다. 운영 새 템플릿·사용자 배포 폼에서 label 13px, 입력값 14px, 높이 40px, 모서리 8px을 확인했다. 운영 390px 뷰포트에서는 입력값 16px·높이 44px, 한 열 배치와 페이지 가로 넘침 없음을 확인했다. CI/E2E 통과는 테스트 환경의 결과이며 운영 데이터 생성·게시·삭제의 수동 검증을 대체하지 않는다.
