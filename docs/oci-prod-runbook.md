...
- **롤백은 관리자 키로 `kubeport-deploy deploy <sha>`** (§3-2). 배포 키 경로로 옛 sha 를 주면
  ...
  - ⚠️ **다중 인스턴스(#190) 이전 커밋으로 롤백할 때** — 옛 이미지는 ui-spec 의 `instances` 를 모른 채 무시해서
    ...
    single 로 렌더될 일이 없다. 대상 확인(읽기 전용, 넓게 잡고 눈으로 거른다 — `instances: "multiple"` 처럼 따옴표나
    flow 형식으로 쓴 버전도 있다):
    ...
    롤백 동안 **어떤 템플릿에도 `instances` 가 든 버전을 게시하지 말고, 기록한 템플릿에서는 새 버전 생성·게시·undeprecate 를
    하지 않는다**(옛 이미지는 `instances` 도 모드 고정도 모른다).
    원인이 고쳐져 다시 앞으로 간 뒤에는 **기록해 둔 목록만** `/undeprecate` 한다 — 원래 deprecated 였던 버전을 되살리지 않도록.
    그 전에 롤백 중 상태가 바뀐 버전을 전부 본다. undeprecate 는 `published_at` 을 갱신하지 않으므로 두 번째 조건으로 게시 중인
    `instances` 버전을 모두 뽑아 기록과 비교한다. 시각에는 **오프셋을 붙인다** — 없으면 세션 TimeZone 으로 해석돼 KST 기준 9시간 어긋난다:
    `SELECT t.name, tv.version, tv.status, tv.published_at FROM template_versions tv JOIN templates t ON t.id = tv.template_id WHERE tv.published_at > '<롤백 시작 시각, 예: 2026-09-13 12:00+09>' OR (tv.status = 'published' AND tv.ui_spec_yaml ~ 'instances');`
    기록한 목록 밖에 결과가 있거나 한 템플릿에 published 버전의 모드가 섞여 있으면 undeprecate 하지 말고 사용자에게 알린다.
    모드가 다른 게시 버전이 남아 있으면 undeprecate 는 400 으로 거절된다.
...
- **데모 비밀번호 회전** — **2단계다(앞에 준비 0단계). 두 번째를 빼면 데모가 멈춘다.**
  ...
  1. [deploy/oci/README.md §7.6](../deploy/oci/README.md) 의 `--set` 목록을 새 `DEMO_PW`/`HASH` 로
     ...
     태그는 올리지 않는다). Dex client secret(`dex.clientSecret`)은 바꾸지 않는다 — §7.6 의 `d=…`·
     `openssl rand … dex_client_secret` 두 줄과 `--set-file dex.clientSecret` 줄은 빼고 실행한다.
     `--reset-then-reuse-values` 가 기존 값을 유지한다.
...
- **데모 계정의 매니페스트 상한**(#350, `demo.policy`): 데모 계정의 릴리스 생성·업데이트·미리보기에서 Job·CronJob job template
...
| 데모에서 배포·수정·삭제가 "실패" 로 끝나고 몇 초 뒤 다시 하면 된다 | 백엔드 액세스 로그에서 `POST /v1/releases`·`PUT`/`DELETE /v1/releases/` 의 `429` 확인 — `title` 은 `rate-limited`, `X-RateLimit-Limit: 30`. 데모 계정 하나를 방문자 전원이 공유해 릴리스 쓰기 버킷(호출자별 분당 30회, #232)이 비었다. 클러스터 호출 전에 거절되는 쓰기(본문 오류·남의 릴리스)는 토큰을 쓰지 않으니 사람 손으로는 넘기 어렵다 — 같은 계정의 반복 호출(스크립트)을 의심한다(계정 공유 문제 #200). 한도는 코드 고정(`routes.go` 의 `Handlers.releaseWrite`) |
| 리셋 Job 의 `seed` 컨테이너가 `rate-limited (attempt N/20), retrying` 을 찍거나 `create release …: 429` 로 실패 | 시드는 방문자와 같은 데모 사용자 계정으로 릴리스를 만들어 그 계정의 쓰기 버킷(분당 30회, #232)을 같이 쓴다. 한두 번의 retrying 은 우연한 충돌이라 정상이다. 20회 끝에 429 로 실패했다면 리셋 시각에 누군가 그 계정으로 쓰기를 계속했다는 뜻 — 이미 wipe 뒤라 데모가 시드 릴리스 없이 남는다. 액세스 로그에서 같은 시각의 `POST`·`PUT`·`DELETE /v1/releases` 반복을 확인하고, 멎은 뒤 §5 "리셋" 의 수동 실행을 한 번 돌린다(근본 해법은 #200 의 로그인 단위 키) |
