...
- **데모 계정의 템플릿 저작**: 기본은 **막혀 있다**(`POST /v1/templates`·버전 게시 `.../versions/:v/publish` → 403 `demo-restricted`).
  초안 작성·미리보기는 열려 있다. 게시까지 막는 이유는 #294 — 공용 데모 계정이 게시한 버전은 다른 방문자가 배포한 Secret 을 파드 로그로 읽을 수 있다.
