...
- **배포 폼의 구역은 데모 세션이면 `demo` 로 시작한다**([#179](https://github.com/shyuni4u/kubeport/issues/179) 에서 고침 — `DEMO_NAMESPACE`). 데모 계정 권한은 `demo` 에만 있으므로(`demo-rbac.yaml`) **폼이 `default` 로 열리거나 권한 확인이 거부하면 회귀다** — #179 에 코멘트하지 말고 새 이슈로 올린다. 구역을 다른 값으로 바꿨을 때 거부 문장이 방문자에게 다음에 뭘 해야 하는지 알려주는지는 여전히 볼 거리다.
