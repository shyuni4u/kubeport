...
- 우측 상단 사용자 메뉴(이메일/역할 배지) 클릭 → **로그아웃** 클릭.
  - 클릭 후 첫 화면(`/`)으로 이동하지 않고 우측 상단에 "로그아웃하지 못했습니다. 아직 로그인된 상태입니다" 알림이 뜨면 로그아웃은 **실패**한 것이다(#166). 닫기로 넘어가지 말고 `unverified` 에 그 문구를 적은 뒤 아래 fetch 방식으로 재시도하고, 응답 status 가 403 이면 그대로 보고한다. 또는
- `javascript_tool` 로 `const r = await fetch('/api/auth/logout', {method: 'POST', redirect: 'manual'}); [r.type, r.status]` 실행 — 성공은 `opaqueredirect`/`0`, 403 은 실패. 그 뒤 `/catalog` 로 이동해 로그인 화면으로 돌아갔는지 확인.

다음 리뷰어가 다른 계정으로 로그인해야 하므로 **로그아웃을 건너뛰면 다음 리뷰어가 잘못된 역할로 리뷰한다.** 로그아웃 후 반드시 `/catalog` 재방문으로 세션이 끊겼는지 확인한다.

## 허용 / 금지

- 허용: 데모 네임스페이스(`demo`) 안에서 UI 가 제공하는 동작 — 기존 데모 템플릿의 초안 작성·미리보기, 배포, 삭제, 팀 페이지 열람. 새 템플릿 생성과 버전 게시·사용 중단·되돌리기는 설치가 `demo.allowTemplateCreate` 를 켜지 않았으면 403 이 정상이다([#294](https://github.com/shyuni4u/kubeport/issues/294)). **만든 것은 직접 지운다** — 리셋은 하루 한 번(KST 06:00)뿐이라 남긴 것이 최대 하루,
...
- **`app-with-config`·`nightly-job` 을 `demo` 에 배포하면 409 가 정상이다.** 두 템플릿은 오브젝트 이름이 고정돼 있고 같은 이름을 시드 릴리스가 이미 쥐고 있어서, "`app-with-config-demo` 릴리스가 이미 쓰고 있다" 는 안내와 함께 거절된다(#161 — 예전엔 조용히 빼앗았고 지우면 시드가 깨졌다). 이름이 고정된 템플릿은 한 네임스페이스에 릴리스를 하나만 둘 수 있다 — 여러 번 두려면 템플릿이 `instances: multiple` 을 쓰거나 이름을 필드로 노출해야 하는데(#190), 시드 템플릿은 둘 다 아니다. **배포 흐름은 비어 있는 `web-app` 으로 검증하고, 이 409 를 결함으로 올리지 말 것.** 단 거절 문장이 어느 릴리스 때문인지·이름을 바꿔도 안 된다는 것을 말하는지는 볼 거리다.
